...
interface StatOption {
    value: keyof AnalyticsGameStat;
    label: string;
}

interface AnalyticsStatsProps {
    analytics: AnalyticsGameStat[];
}
...
export default function PlayerAnalyticsGraph({ analytics }: AnalyticsStatsProps) {
    ...
    const [selectedTeamStat, setSelectedTeamStat] = useState<string>('avg_points_scored');

    useEffect(() => {
        // Generate unique player options from the analytics data
        ...
        // Process chart data whenever the selected players or stat changes
        const data: ChartData[] = analytics.map(stat => {
          const playerScores: PlayerScores = {};
          selectedPlayers.forEach(player => {
            const statValue = stat[selectedStat as keyof AnalyticsGameStat];
            playerScores[player.label] = stat.full_name === player.label 
              ? (typeof statValue === 'number' ? statValue : null) 
              : undefined;
          });
    
          const teamStatValue = stat[selectedTeamStat as keyof AnalyticsGameStat];
          return {
              game_date: new Date(stat.game_date).toLocaleDateString(),
      ...
      const teamStatOptions = [
        { value: 'avg_points_scored', label: 'Average Points Scored' },
      ...
      const handleStatChange = (
        ...
        if (selectedOption) {
          setSelectedStat(selectedOption.value as keyof AnalyticsGameStat);
        }
      };   
    ...
    const handleTeamStatChange = (
        ...
        if (selectedOption) {
            setSelectedTeamStat(selectedOption.value as keyof AnalyticsGameStat);
        }
    };    
    ...
    return (
        <div>
            <h1 className="text-lg font-bold text-center">Game Stats Comparison</h1>
            <h3>Player:</h3>
            <Select
